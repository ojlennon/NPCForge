  ...
  try {
    ...
    session.onEvent("contentEnd", async (data) => {
      if (data.type === "AUDIO" && data.stopReason === "END_TURN") {
        ...
        console.log(
          "To convert to WAV: ffmpeg -f s16le -ar 24000 -ac 1 -i output-audio.raw output-audio.wav"
        );
        audioResponse = "";
    ...
    session.onEvent("streamComplete", () => {
      ...
      if (transcription) {
        ...
        console.log(
          `Transcription saved to: ${join(OUTPUT_DIR, "transcription.txt")}`
        );
      }
    ...
    console.log(
      "Starting event sequence: Prompt Start -> System Prompt -> Audio Start"
    );
    ...
    console.log(
      "All audio chunks sent, waiting before ending input content..."
    );
    ...
    console.log(
      "Waiting for server to process all audio and generate responses..."
    );
  } catch (error) {
